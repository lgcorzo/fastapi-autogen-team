...
## 2026-02-27 - Prompt Injection Bypass via Line Endings

**Vulnerability:** The `sanitize_for_prompt` function was only checking for `\n` based delimiters, allowing attackers to bypass the check using `\r` or `\r\n` (CRLF) sequences if the downstream system or LLM normalizes them.
**Learning:** Sanitization checks must account for all variations of control characters, especially line endings which can vary by platform or protocol.
**Prevention:** Normalize line endings (e.g., replace `\r\n` and `\r` with `\n`) before applying structural sanitization checks to ensure no bypass is possible.
