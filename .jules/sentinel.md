...
## 2026-01-25 - Information Leakage in Agent Tools
**Vulnerability:** The `tool.py` module, used by LLM agents, was catching exceptions and returning `f"Error: {e}"`. This allowed sensitive details (like database credentials or internal URLs) to leak into the agent's context and potentially be relayed to the user.
**Learning:** Security controls must extend to internal tools and helper functions, not just API endpoints. Agent tools act as a bridge between internal systems and the LLM.
**Prevention:** Ensure all tool return values are sanitized on error. Return generic messages to the agent, while logging full details securely.
