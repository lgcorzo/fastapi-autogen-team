...
async def async_search(query: str, timeout: float = 10.0):
    logger.info(f"Ejecutando búsqueda para: {sanitize_log_input(query)}")
    ...
    try:
        ...
        r2r_result = (
            results[0] if not isinstance(results[0], Exception) else "An error occurred while retrieving R2R results."
        )
        jira_result = (
            results[1] if not isinstance(results[1], Exception) else "An error occurred while retrieving Jira results."
        )
...
def get_jira_results(query: str):
    ...
    logger.info(f"Ejecutando consulta Jira JQL para: {sanitize_log_input(query)}")
