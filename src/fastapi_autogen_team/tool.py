...
async def async_search(query: str, timeout: float = 10.0):
    ...
    try:
        ...
        if isinstance(results[0], Exception):
            logger.error(f"R2R error or timeout: {results[0]}", exc_info=True)
            r2r_result = "An internal error occurred while retrieving R2R results."
        else:
            r2r_result = results[0]

        if isinstance(results[1], Exception):
            logger.error(f"Jira error or timeout: {results[1]}", exc_info=True)
            jira_result = "An internal error occurred while retrieving Jira results."
        else:
...
def safe_get_r2r_results(query: str):
    try:
        ...
        logger.error(f"Error al obtener resultados de R2R: {e}", exc_info=True)
        return "An internal error occurred while retrieving R2R results."
...
def safe_get_jira_results(query: str):
    try:
        ...
        logger.error(f"Error al obtener resultados de Jira: {e}", exc_info=True)
        return "An internal error occurred while retrieving Jira results."
