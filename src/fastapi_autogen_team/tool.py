...
async def async_search(query: str, timeout: float = 10.0):
    ...
    try:
        ...
        if isinstance(results[0], Exception):
            logger.error(f"R2R timeout/error: {results[0]}", exc_info=True)
            r2r_result = "An internal error occurred during R2R search."
        else:
            r2r_result = results[0]

        if isinstance(results[1], Exception):
            logger.error(f"Jira timeout/error: {results[1]}", exc_info=True)
            jira_result = "An internal error occurred during Jira search."
        else:
...
def safe_get_r2r_results(query: str):
    try:
        ...
        return "An internal error occurred during R2R search."
...
def safe_get_jira_results(query: str):
    try:
        ...
        return "An internal error occurred during Jira search."
