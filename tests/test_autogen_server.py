...
def test_serve_autogen_non_streaming(mock_autogen_workflow):
    """Test serve_autogen with non-streaming response."""
    mock_autogen_workflow.run.return_value = MagicMock(chat_id=TEST_CHAT_ID, chat_history=[{"role": "assistant", "content": "Hi!"}], cost={})
    result = serve_autogen(TEST_INPUT)
    assert isinstance(result, dict)
    assert result["id"] == TEST_CHAT_ID
    assert len(result["choices"]) == 1
    mock_autogen_workflow.run.assert_called_once()
...
def test_serve_autogen_streaming(mock_autogen_workflow):
    ...
    with patch("fastapi_autogen_team.autogen_server.StreamingResponse") as MockStreamingResponse:
        serve_autogen(Input(model=MODEL_NAME, messages=[{"role": "user", "content": TEST_MESSAGE}], stream=True))
        MockStreamingResponse.assert_called_once()
    mock_autogen_workflow.set_queue.assert_called_once()

def test_serve_autogen_exception(mock_autogen_workflow):
    """Test serve_autogen with an exception raised during workflow execution."""
    mock_autogen_workflow.run.side_effect = ValueError("Test error")
    with pytest.raises(HTTPException) as exc_info:
