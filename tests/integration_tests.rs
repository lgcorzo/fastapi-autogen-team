...
use fastapi_autogen_team::{create_app, AppState, agents::AgentTeam, data_model::{Input, Message, ContentType}};
...
async fn test_chat_completions_route() {
    ...
    // Mock the 3 stages in sequence with Rig responses_api compatible JSON
    ...
    let body = response.into_body().collect().await.unwrap().to_bytes();
    let body: Value = serde_json::from_slice(&body).unwrap();
    assert!(body["choices"][0]["message"]["content"].as_str().unwrap().contains("TERMINATE"));
}

#[tokio::test]
async fn test_chat_stream_completion_success() {
    let mut server = Server::new_async().await;
    let url = server.url();
    ...
    let _m1 = server.mock("POST", mockito::Matcher::Any)
        ...
        .with_body(json!({
            ...
            "status": "completed",
            "model": "gpt-4o",
            "output": [{"type": "message", "role": "assistant", "content": [{"type": "output_text", "text": "query1"}]}]
        }).to_string())
        .create_async().await;
    ...
    let _m2 = server.mock("POST", mockito::Matcher::Any)
        ...
        .with_body(json!({
            ...
            "status": "completed",
            "model": "gpt-4o",
            "output": [{"type": "message", "role": "assistant", "content": [{"type": "output_text", "text": "search result"}]}]
        }).to_string())
        .create_async().await;

    // Mock 3: QA (Streaming - Rig uses SSE for streaming)
    // NOTE: Rig's stream_prompt usually expects a chunked response.
    // For simplicity, we just mock the first chunk.
    let _m3 = server.mock("POST", mockito::Matcher::Any)
        .with_status(200)
        .with_header("content-type", "text/event-stream")
        .with_body("data: {\"id\":\"abc\",\"object\":\"chat.completion.chunk\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"STREAMED ANSWER TERMINATE\"},\"finish_reason\":null}]}\n\n")
        .create_async().await;
    ...
    assert!(body_str.contains("STREAMED ANSWER TERMINATE"));
}
